...
def main():
    ...
    try:
        pid,code,wall=launch_stage([str(JULIA),"--startup-file=no",f"--project={ROOT}",
            ...
            str(time.time()+args.seconds),str(spool),str(output/"options.json")],
            deadline=time.time()+args.seconds+5,log_path=output/"console.log",
            memory_path=output/"memory.jsonl",progress_path=output/"progress")
        result={"process_exit":code,"pid":pid,"process_wall_seconds":wall,"error":None}
