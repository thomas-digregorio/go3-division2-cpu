...
@testset "Optional analytic center preserves enumerated root MIP optima" begin
    for n in (5,7,9,11)
        expected=Inf
        for bits in 0:(2^n-1)
        ...
        for analytic in (true,false)
            ...
            path=joinpath(directory,"n$(n)_center$(analytic).log")
            try
...
println("NATIVE_ROOT_MEMORY_MATH_PASS ",JSON.json(Dict("tiny_only"=>true,"full_case_runs"=>0,
    "known_optimum_fixtures"=>4,"feasible_solves"=>8,"infeasible_solves"=>2,"identity"=>identity)))
